...
class ShellCommandProvider : ContentProvider() {
    ...
    override fun call(method: String, arg: String?, extras: Bundle?): Bundle? {
        // Only allow calls from system UID (SystemUI / system_server)
        ...
        Thread {
            try {
                if (runAsRoot) {
                    ...
                    if (exitCode == 0) {
                        output.trim().takeIf { it.isNotBlank() }?.let { showToast(it.take(200)) }
                    } else {
                        showToast(context!!.getString(R.string.toast_command_failed, output.trim().take(200)))
                    }
                }
            } catch (e: Exception) {
